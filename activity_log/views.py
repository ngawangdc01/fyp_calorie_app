...
#                 # weight_entry = WeightHistory.objects.filter(user=request.user, recorded_at=selected_date).first()
...
#                     # If today, check if user manually entered weight
#                     if input_weight:
#                         # Overwrite today's history
#                         WeightHistory.objects.update_or_create(
#                             user=request.user,
#                             recorded_at=today,
#                             defaults={'weight_kg': input_weight}
#                         )
#                         new_log.weight_kg = input_weight
#                     elif weight_entry:
#                         new_log.weight_kg = weight_entry.weight_kg
#                     else:
#                         # Use profile weight and log it into history
#                         new_log.weight_kg = request.user.weight
...
#                         new_log.weight_kg = weight_entry.weight_kg
#                     else:
#                         new_log.weight_kg = request.user.weight  # fallback
...
#                 return redirect(f"{reverse('log_activity')}?date={new_log.logged_date}#add-exercise")
...
#         if selected_date != today:
#             # weight_entry = WeightHistory.objects.filter(user=request.user, recorded_at=selected_date).first()
#             weight_entry = WeightHistory.objects.filter(  # ← CHANGE THIS LINE TOO
#                 user=request.user,
#                 recorded_at__lte=selected_date
#             ).order_by('-recorded_at').first()
#             form.fields['weight_kg'].initial = weight_entry.weight_kg if weight_entry else request.user.weight
#             form.fields['weight_kg'].label = f"Weight on {selected_date.strftime('%d-%b-%Y')} (kg)"
#         else:
#             del form.fields['weight_kg']
